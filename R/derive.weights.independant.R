`derive.weights.independant` <- function(set=NULL, sis = 1000) {
	...
	for(x in 1 :length(data[,1])) {
		su = sum(data[x,]); msc[x,1] = (sum(cen*data[x,])/su); msc[x,2] = (sum(dif*data[x,])/su); msc[x,3] = (sum(wav*data[x,])/su);
		score[x]=(msc[x,1]+msc[x,2]+msc[x,3])/3
	} 
