   ...
   public static double[] rtab(double[] p1, double[] p2) {  
		...
		for(int i=1;i<p2.length-1;i++) {
			double[] temp = new double[5]; int pin = i; if(pin>=p2.length-4) { pin = pin-3; }
          	...
          	temp = new double[25]; pin2 = i-25; if(pin2<0) { pin2 =0; }
			...
			double tmd2 = median(temp); double las = 0;		
			if(Math.abs(p2[i])<r) {
 				pp[i] = p2[i]-p1[i];	
 				if(Math.abs(p1[i]-tmd2) < Math.abs(rr*Math.abs(tmd1*(factor)))) { las = p1[i]; }
				if(Math.abs(p1[i]-tmd2) > Math.abs(rr*Math.abs(tmd1*factor))) { pp[i] = p2[i]-las; }		
			...
			if(Math.abs(tmd1)>(ar*factor) | Math.abs(rtmd1)>(ar*factor) | Math.abs(tmd15)>(ar*factor) | Math.abs(rtmd15)>(ar*factor)) { pp[i]=p2[i]; }
			if(Math.abs(pp[i]-pp[i-1])>Math.abs(rr*Math.abs(tmd1*(factor)))) { pp[i-1] = p2[i-1]-las; }
			if(Math.abs(pp[i]-p2[i])>rr) { pp[i] = p2[i]-las; }
			if(pp[i]>0 & p2[i]<0 | pp[i]<0 & p2[i]>0) { if(Math.abs(pp[i])>ar*3) { pp[i]=p2[i]; } }	
			//if(Math.abs(pp[i]-p2[i])>ar*2.5) { pp[i]=p2[i]; }
			//if(Math.abs(median(new double[]{p2[i-1], p2[i], p2[i+1]}))>(ar*factor)) { pp[i]=p2[i]; }
		}
   	return(pp);
   }
